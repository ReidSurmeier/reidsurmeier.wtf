...
import { txtTiles } from "../../data";
import { notFound } from "next/navigation";

export function generateStaticParams() {
  return txtTiles.map((t) => ({ id: t.id }));
}
...
export async function generateMetadata({
  ...
  const tile = txtTiles.find((t) => t.id === id);
  if (!tile) return {};

  return {
    title: tile.title,
    description: tile.subtitle || undefined,
    alternates: { canonical: `/txt/${tile.id}` },
    openGraph: {
      title: tile.title,
      description: tile.subtitle || undefined,
...
export default async function TxtPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
